#include <iostream>
#include <math.h> // donde viene incluido NAN
using namespace std;
...
private:
    ...
    bool CoordCorrectas(double abs, double ord){
        return abs == ord;
    }

    bool LongCorrecta(double lado){
        return lado > 0;
    }

public:
    /*Constructor inicializa los datos miembro "x, y, longitud"
    haciendo que esté en estado válido desde el primer momento
    en el que se crea el objeto*/

    Cuadrado(double abs, double ord, double lado){
        if(CoordCorrectas(abs, ord) && LongCorrecta(lado)){
            x = abs;
            y = ord;
            longitud = lado;
        }else

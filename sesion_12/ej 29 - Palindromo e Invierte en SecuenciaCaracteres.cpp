...
public:
   ...
   void Invierte(){            // Al ser un método que modifica un objeto
      int izda = 0;            // no devuelve ningún valor (sin return)
      int dcha = util - 1;
   ...
   /* Si se decidiera cambiar el método IntercambiaComponentes al
      ámbito público, la implementación sería la misma. Lo único
      que cambia es que no se puede llamar directamente al método
      desde el main (solo dentro de la clase) en caso de que sea
      privado.
   */
};
...
int main(){
   ...
   cout << "-----------MÉTODOS PALÍNDROMO E INVIERTE----------" << endl;
   cout << "Introduzca la cadena de caracteres (term: '#'): " << endl;
   ...
   // Llamada al método Palindromo e Invierte

   es_palindromo = secuencia.EsPalindromo();
   secuencia.Invierte();   // <- No se le puede asignar a una variable ya
   inversa = secuencia;    //    que NO es un valor (es un método void)
